package abdulkadir.stepview.ui;

import abdulkadir.stepview.R;
...
public class StepView extends View {
    ...
    private boolean isJustGo = false;
    ...
    @DisplayMode private int          displayMode   = DISPLAY_MODE_WITH_TEXT;
    ...
    @AnimationType private                  int   animationType;
    @ColorInt private                       int   selectedCircleColor;
    @Dimension private                      int   selectedCircleRadius;
    @ColorInt private                       int   selectedTextColor;
    @ColorInt private                       int   doneCircleColor;
    @Dimension private                      int   doneCircleRadius;
    @ColorInt private                       int   doneTextColor;
    @ColorInt private                       int   nextTextColor;
    @Dimension private                      int   stepPadding;
    @ColorInt private                       int   nextStepLineColor;
    @ColorInt private                       int   doneStepLineColor;
    @Dimension private                      int   stepLineWidth;
    @Dimension(unit = Dimension.SP) private float textSize;
    @Dimension private                      int   textPadding;
    private                                 int   selectedStepNumberColor;
    @Dimension(unit = Dimension.SP) private float stepNumberTextSize;
    @ColorInt private                       int   doneStepMarkColor;
    ...
    private Paint pCircleOut;
    private Paint textNotificationPaint;
    private Rect  notificationBounds;
    ...
    private void applyStyles(Context context,
                             AttributeSet attrs,
                             int defStyleAttr) {
        TypedArray ta = context.obtainStyledAttributes(attrs,
                                                       ...
                                                       defStyleAttr,
                                                       R.style.StepView);
        selectedCircleColor = ta.getColor(
            R.styleable.StepView_sv_selectedCircleColor, 0);
        selectedCircleRadius = ta.getDimensionPixelSize(
            R.styleable.StepView_sv_selectedCircleRadius, 0);
        selectedTextColor = ta.getColor(
            R.styleable.StepView_sv_selectedTextColor, 0);
        selectedStepNumberColor = ta.getColor(
            R.styleable.StepView_sv_selectedStepNumberColor, 0);
        doneStepMarkColor = ta.getColor(
            R.styleable.StepView_sv_doneStepMarkColor, 0);
        doneCircleColor = ta.getColor(
            R.styleable.StepView_sv_doneCircleColor, 0);
        doneCircleRadius = ta.getDimensionPixelSize(
            R.styleable.StepView_sv_doneCircleRadius, 0);
        ...
        stepPadding = ta.getDimensionPixelSize(
            R.styleable.StepView_sv_stepPadding, 0);
        nextStepLineColor = ta.getColor(
            R.styleable.StepView_sv_nextStepLineColor, 0);
        doneStepLineColor = ta.getColor(
            R.styleable.StepView_sv_doneStepLineColor, 0);
        stepLineWidth = ta.getDimensionPixelSize(
            R.styleable.StepView_sv_stepLineWidth, 0);
        textPadding = ta.getDimensionPixelSize(
            R.styleable.StepView_sv_textPadding, 0);
        stepNumberTextSize = ta.getDimension(
            R.styleable.StepView_sv_stepNumberTextSize, 0);
        textSize = ta.getDimension(R.styleable.StepView_sv_textSize, 0);
        animationDuration = ta.getInteger(
            R.styleable.StepView_sv_animationDuration, 0);
        animationType = ta.getInteger(
            R.styleable.StepView_sv_animationType, 0);
        stepsNumber = ta.getInteger(R.styleable.StepView_sv_stepsNumber, 0);
        CharSequence[] descriptions = ta.getTextArray(
            R.styleable.StepView_sv_steps);
        ...
        Drawable background = ta.getDrawable(
            R.styleable.StepView_sv_background);
        if (background != null) {
    ...
    public void go(int step,
                   ...
                   boolean isJustGo) {
        go(step, animate);
        this.isJustGo = isJustGo;
    }

    private void justGoStep(Canvas canvas,
                            int step,
                            int circleCenterX,
                            int circleCenterY) {
        ...
        int radius;
        if (state == ANIMATE_STEP_TRANSITION && (animationType == ANIMATION_CIRCLE
            || animationType == ANIMATION_ALL) && nextAnimatedStep < currentStep) {
        ...
        if (step > currentStep) {
            if (animationType == ANIMATION_LINE) {
                ...
                animator = ValueAnimator.ofInt(
                    0, (endLinesX[i] - startLinesX[i]
                        + selectedCircleRadius) / 2);
            }
    ...
    private int measureHeight(int heightMeasureSpec) {
        ...
        int desiredSize = getPaddingTop()
            + getPaddingBottom()
            + (Math.max(
            selectedCircleRadius, doneCircleRadius)) * 2
    ...
    private int measureStepsHeight() {
        ...
        for (int i = 0; i < steps.size(); i++) {
            ...
            textLayouts[i] = new StaticLayout(text, textPaint, getMeasuredWidth() / steps.size(),
                                              alignment, 1, 0, true);
    ...
    private int getCircleY() {
        ...
        int maxItemHeight = getMaxTextHeight()
            + Math.max(selectedCircleRadius, doneCircleRadius)
            + textPadding;
    ...
    private int[] getCirclePositions() {
        ...
        float spaceLeft =
            isRtl() ? result[0] - result[stepsCount - 1] : result[stepsCount - 1] - result[0];
    ...
    private int getStartCirclePosition() {
        ...
        if (displayMode == DISPLAY_MODE_WITH_TEXT) {
            if (isRtl()) {
                result = getMeasuredWidth() - getPaddingRight() - Math.max(
                    getMaxLineWidth(textLayouts[0]) / 2, selectedCircleRadius);
            } else {
                result = getPaddingLeft() + Math.max(
                    getMaxLineWidth(textLayouts[0]) / 2, selectedCircleRadius);
            }
        } else {
            if (isRtl()) {
    ...
    private int getEndCirclePosition() {
        ...
        if (displayMode == DISPLAY_MODE_WITH_TEXT) {
            if (isRtl()) {
                result = getPaddingLeft() + Math.max(
                    getMaxLineWidth(last(textLayouts)) / 2, selectedCircleRadius);
            } else {
                result = getMeasuredWidth() - getPaddingRight() - Math.max(
                    getMaxLineWidth(last(textLayouts)) / 2, selectedCircleRadius);
            }
    ...
    protected void onDraw(Canvas canvas) {
        ...
        for (int i = 0; i < stepSize; i++) {
            drawStep(canvas, i, circlesX[i], circlesY,isJustGo);
        }

        for (int i = 0; i < startLinesX.length; i++) {
            if (state == ANIMATE_STEP_TRANSITION
                && i == nextAnimatedStep - 1
                && nextAnimatedStep > currentStep
                && (animationType == ANIMATION_LINE || animationType == ANIMATION_ALL)) {
                int animatedX = (int) (startLinesX[i] + animatedFraction * (endLinesX[i]
                    - startLinesX[i]));
                drawLine(canvas, startLinesX[i], animatedX, circlesY, true);
                drawLine(canvas, animatedX, endLinesX[i], circlesY, false);
            } else if (state == ANIMATE_STEP_TRANSITION
                && i == nextAnimatedStep
                && nextAnimatedStep < currentStep
                && (animationType == ANIMATION_LINE || animationType == ANIMATION_ALL)) {
                int animatedX = (int) (endLinesX[i] - animatedFraction * (endLinesX[i]
                    - startLinesX[i]));
                drawLine(canvas, startLinesX[i], animatedX, circlesY, true);
                drawLine(canvas, animatedX, endLinesX[i], circlesY, false);
        ...
        if (isChecked){
            drawCheckMark(canvas,circlesX[currentStep-1],circlesY);
        }
    }
    ...
    private void updateNotificationCounter(Canvas canvas,
                                           int step){
        final String text = displayMode == DISPLAY_MODE_WITH_TEXT ? steps.get(step) : "";
        final boolean isSelected = step == currentStep;
        final boolean isDone = done ? step <= currentStep : step < currentStep;
    ...
    private void drawStep(Canvas canvas,
                          int step,
                          int circleCenterX,
                          int circleCenterY,boolean isJustGo) {
        final String text = displayMode == DISPLAY_MODE_WITH_TEXT ? steps.get(step) : "";
        final boolean isSelected = step == currentStep;
        final boolean isDone = done ? step <= currentStep : step < currentStep;
        final String number = String.valueOf(step + 1);
    ...
    private void drawText(Canvas canvas,
        ...
        layout.draw(canvas);
        canvas.restore();
        mText = notifications.get(step);
        if (mText != null && !mText.equals("0")) {
            Rect textBound = new Rect();
            Paint notificationPaint;
            textPaint.getTextBounds(mText, 0, mText.length(), notificationBounds);
            textPaint.getTextBounds(text, 0, text.length(), textBound);
            notificationPaint = textPaint;
            notificationPaint.setColor(Color.RED);
            notificationPaint.setTextSize(13);

            //int textWidth = textBound.right - textBound.left;
            //int notificationX = textWidth + circlesX[step] - 13;
            ////int notificationX = textWidth +circlesX[step];
            //int notificationY = y;
            //int textHeight = textBound.bottom - textBound.top;
            //notificationY = y + textHeight;
            ...
            canvas.drawCircle(circlesX[step], 105, 11, pCircle);
            //canvas.drawCircle(notificationX, notificationY -2, notificationBounds.height() -2, pCircleOut);
            canvas.drawText(mText, circlesX[step], 109, notificationPaint);
        }
    }

    private void init(@Nullable AttributeSet attrs) {
        mTextBackgroundColor = Color.RED;
        mTextColor = Color.WHITE;
        mTextSize = 1;
        pCircle = new Paint();
        pCircle.setAntiAlias(true);
        pCircle.setColor(mTextBackgroundColor);
        pCircle.setStyle(Paint.Style.FILL);

        pCircleOut = new Paint();
        pCircleOut.setAntiAlias(true);
        pCircleOut.setColor(mTextColor);
        pCircleOut.setStyle(Paint.Style.STROKE);
        pCircleOut.setStrokeWidth(1);

        textNotificationPaint = new Paint(Paint.ANTI_ALIAS_FLAG | Paint.DITHER_FLAG);
        textNotificationPaint.setColor(mTextColor);
        //textPaint.setTextSize((int) TypedValue.applyDimension(TypedValue.COMPLEX_UNIT_SP, 20, getResources().getDisplayMetrics()));
        textNotificationPaint.setTextSize(mTextSize);
        textNotificationPaint.setTextAlign(Paint.Align.CENTER);
        notificationBounds = new Rect();
    }
    ...
    private void drawCheckMark(Canvas canvas,
        ...
        Rect bounds = new Rect((int) (circleCenterX - width * 4.5),
                               (int) (circleCenterY - width * 3.5),
                               (int) (circleCenterX + width * 4.5),
                               (int) (circleCenterY + width * 3.5));
        canvas.drawLine(bounds.left + 0.5f * width, bounds.bottom - 3.25f * width,
                        bounds.left + 3.25f * width, bounds.bottom - 0.75f * width, paint);
        canvas.drawLine(bounds.left + 2.75f * width, bounds.bottom - 0.75f * width,
                        bounds.right - 0.375f * width, bounds.top + 0.75f * width, paint);
    }

    public void makeCheck(boolean isCheck){
    ...
    public class State {
        ...
        @AnimationType private                  int          animationType           = this.animationType;
        @ColorInt private                       int          selectedCircleColor     = this.selectedCircleColor;
        @Dimension private                      int          selectedCircleRadius    = this.selectedCircleRadius;
        @ColorInt private                       int          selectedTextColor       = this.selectedTextColor;
        @ColorInt private                       int          doneCircleColor         = this.doneCircleColor;
        @Dimension private                      int          doneCircleRadius        = this.doneCircleRadius;
        @ColorInt private                       int          doneTextColor           = this.doneTextColor;
        @ColorInt private                       int          nextTextColor           = this.nextTextColor;
        @Dimension private                      int          stepPadding             = this.stepPadding;
        @ColorInt private                       int          nextStepLineColor       = this.nextStepLineColor;
        @ColorInt private                       int          doneStepLineColor       = this.doneStepLineColor;
        @Dimension private                      int          stepLineWidth           = this.stepLineWidth;
        @Dimension(unit = Dimension.SP) private float        textSize                = this.textSize;
        @Dimension private                      int          textPadding             = this.textPadding;
        @ColorInt private                       int          selectedStepNumberColor = this.selectedStepNumberColor;
        @Dimension(unit = Dimension.SP) private float        stepNumberTextSize      = this.stepNumberTextSize;
        @ColorInt private                       int          doneStepMarkColor       = this.doneStepMarkColor;
        ...
        public StepView.State animationDuration(int animationDuration) {
            this.animationDuration = animationDuration;
            return this;
        }

        public StepView.State steps(List<String> steps) {
            this.steps = steps;
            return this;
        }

        public StepView.State stepsNumber(int stepsNumber) {
            this.stepsNumber = stepsNumber;
            return this;
        }

        public StepView.State typeface(Typeface typeface) {
